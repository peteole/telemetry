...
class ArtificialHorizon extends Instrument {
    ...
    setRxInput(input) {
        this.thr = input[2];
        this.elv = input[1] - input[0] + 0.5;
        this.ail = input[0] + input[1] - 0.5;
        this.rud = input[3];
    }
    resize() {
        super.resize();
    ...
    draw() {
        ...
        this.canvasDrawer.setTransform(1, 0, 0, 1, 0, 0);
        //draw inputs
        this.canvasDrawer.rect(this.width * (1 - this.ail) - 5, this.height * this.elv - 5, 10, 10);
        this.canvasDrawer.stroke();
        this.canvasDrawer.rect(0, this.height * (1 - this.thr) - 5, 10, 10);
        this.canvasDrawer.stroke();
    ...
    draw() {
        ...
        if (this.horizonal) {
            for (var value = minValueToDraw; value <= maxValueToDraw; value += step) {
                let coordinate = this.valueToX(value);
                this.canvasDrawer.beginPath();
            ...
            for (let value = minValueToDraw; value <= maxValueToDraw; value += step) {
                let coordinate = this.valueToY(value);
                this.canvasDrawer.beginPath();
